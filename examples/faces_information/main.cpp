...
{
    Vector3D ll(-10, -10, -10);
    Vector3D ur(10, 10, 10);

    MadVoro::Voronoi3D<Vector3D> diagram(ll, ur);

    std::vector<Vector3D> points {{ -0.115, 4.484, 0.688 },
                                   { -6.121, 6.335, -9.093 },
                                   { -5.529, -7.464, 2.459 },
                                   { -3.448, -7.794, -6.939 },
                                   { -3.231, 8.213, -7.829 },
    ...
    auto boundingBox = diagram.GetBoxCoordinates();
    ...
    std::vector<Vector3D> facesVertices = diagram.GetFacePoints();

    std::cout << "And these are the faces: " << std::endl;
    for(const size_t &faceIdx : faces)
    {
        Vector3D centerOfMass = diagram.FaceCM(faceIdx);
        const std::pair<size_t, size_t> neighbors = diagram.GetFaceNeighbors(faceIdx);
