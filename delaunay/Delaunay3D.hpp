...
bool InsideBigTetra(PointT const& tocheck, vector<PointT> const& cor, size_t Norg)
{
    int sum = 0;
...
#pragma omp simd reduction(+:sum)
    ...
    for(size_t i = 0; i < 4; ++i) {
        PointT normal = CrossProduct(cor[Norg + (1 + i) % 4] - cor[Norg + i], cor[Norg + (2 + i) % 4] - cor[Norg + i]);
...
bool PlaneLineIntersection(std::array<PointT, 3> &plane, PointT const& A, PointT const& B, PointT &res)
{
    plane[1] -= plane[0];
    plane[2] -= plane[0];
    PointT N = CrossProduct(plane[1], plane[2]);
...
std::pair<std::size_t, double> InTriangle(std::array<PointT, 3> &triangle, PointT &p)
{
...
public:
   ...
   */ 
  void Build(vector<PointT> const& points, PointT const& maxv, PointT const& minv, std::vector<size_t> &order);

  /*! \brief Add new points and rebuild
    \param points New points
   */
  void BuildExtra(vector<PointT> const& points);
...
private:
  ...
  std::array<PointT, 3> b3_temp_,b3_temp2_;
  std::array<PointT, 4> b4_temp_;
  std::array<PointT, 5> b5_temp_;
...
template <typename PointT>
Delaunay3D<PointT>& Delaunay3D<PointT>::operator=(Delaunay3D<PointT> const& other)
{
    if(this == &other)
        return *this;
...
template <typename PointT>
Delaunay3D<PointT>::Delaunay3D(Delaunay3D<PointT> const& other)
    : tetras_(other.tetras_),
      points_(other.points_),
      empty_tetras_(other.empty_tetras_),
      Norg_(other.Norg_),
      outside_neighbor_(other.outside_neighbor_),
      b3_temp_(std::array<PointT, 3>()),
      b3_temp2_(std::array<PointT, 3>()),
      b4_temp_(std::array<PointT, 4>()),
      b5_temp_(std::array<PointT, 5>()),
      /*b4s_temp_(std::array<std::size_t, 4>()),*/
      /*b4s_temp2_(std::array<std::size_t, 4>()),*/
      b8s_temp_(std::array<std::size_t, 8>()),
      to_check_(vector<std::size_t>()),
      last_checked_(0),
      tet_temp0_(Tetrahedron()),
      tet_temp1_(Tetrahedron()),
      newtet_(Tetrahedron())
{}

template <typename PointT>
void Delaunay3D<PointT>::flip23(std::size_t tetra0, std::size_t tetra1, std::size_t location0, bool flat_check)
{
    ...
    if(flat_check) {
        ...
        if(MadVoro::ContainerOps::close2zero(orient)) {
            std::size_t my_loc = 0;
...
template <typename PointT>
void Delaunay3D<PointT>::flip32(std::size_t tetra0, std::size_t tetra1, std::size_t location0, std::size_t shared_loction, bool flat_check)
{
...
template <typename PointT>
void Delaunay3D<PointT>::flip44(std::size_t tetra0, std::size_t tetra1, std::size_t location0, std::size_t neigh0, std::size_t neigh1)
{
...
template <typename PointT>
Delaunay3D<PointT>::Delaunay3D()
    : tetras_(vector<Tetrahedron>()),
      points_(vector<PointT>()),
      empty_tetras_(boost::container::flat_set<size_t>()),
      Norg_(0),
      outside_neighbor_(0),
      b3_temp_(std::array<PointT, 3>()),
      b3_temp2_(std::array<PointT, 3>()),
    ...
    std::vector<size_t>& order)
{
    ...
    points_.push_back(PointT(cx,                        cy + R_base,       z_base));
    points_.push_back(PointT(cx + R_base * sqrt3_half,  cy - R_base / 2.0, z_base));
    points_.push_back(PointT(cx - R_base * sqrt3_half,  cy - R_base / 2.0, z_base));
    points_.push_back(PointT(cx, cy, z_top));
    // Create large tetra
...
template <typename PointT>
void Delaunay3D<PointT>::output(string const& filename) const
{
...
template <typename PointT>
std::size_t Delaunay3D<PointT>::FindThirdNeighbor(std::size_t tetra0, std::size_t tetra1)
{
    for(size_t i = 0; i < 4; ++i) {
...
template <typename PointT>
void Delaunay3D<PointT>::ExactFlip(std::size_t tetra0, std::size_t tetra1, std::size_t p)
{
    ...
    if(MadVoro::ContainerOps::close2zero(test0))
        in_counter++;
    b4_temp_[3] = points_[other_point];
    double test1 = orient3d(b4_temp_);
    if(MadVoro::ContainerOps::close2zero(test1))
        flat_counter++;
    if(test0 * test1 > 0)
        out_check[0] = 0;
    else {
    ...
    if(MadVoro::ContainerOps::close2zero(std::abs(test0)))
        in_counter++;
    b4_temp_[3] = points_[other_point];
    test1 = orient3d(b4_temp_);
    if(MadVoro::ContainerOps::close2zero(test1))
...
template <typename PointT>
void Delaunay3D<PointT>::FindFlip(std::size_t tetra0, std::size_t tetra1, std::size_t p, size_t p_loc, size_t other_point_loc)
{
...
template <typename PointT>
void Delaunay3D<PointT>::InsertPoint(std::size_t index)
{
    if (!InsideBigTetra(points_[index], points_, Norg_)) {
        MadVoro::Exception::MadVoroException eo("Point is outside the bounding tetrahedron");
        eo.addEntry("point index", index);
        eo.addEntry("x", points_[index].x);
        eo.addEntry("y", points_[index].y);
        eo.addEntry("z", points_[index].z);
        eo.addEntry("big tetra v0 x", points_[Norg_].x);
        eo.addEntry("big tetra v0 y", points_[Norg_].y);
        eo.addEntry("big tetra v0 z", points_[Norg_].z);
        eo.addEntry("big tetra v1 x", points_[Norg_ + 1].x);
        eo.addEntry("big tetra v1 y", points_[Norg_ + 1].y);
        eo.addEntry("big tetra v1 z", points_[Norg_ + 1].z);
        eo.addEntry("big tetra v2 x", points_[Norg_ + 2].x);
        eo.addEntry("big tetra v2 y", points_[Norg_ + 2].y);
        eo.addEntry("big tetra v2 z", points_[Norg_ + 2].z);
        eo.addEntry("big tetra v3 x", points_[Norg_ + 3].x);
        eo.addEntry("big tetra v3 y", points_[Norg_ + 3].y);
        eo.addEntry("big tetra v3 z", points_[Norg_ + 3].z);
        throw eo;
    }
...
template <typename PointT>
std::size_t Delaunay3D<PointT>::Walk(std::size_t point, std::size_t first_guess)
{
    bool good = false;
        ...
        for(size_t i = 0; i < 4; ++i) {
            ...
            if((orient3d(b4_temp_) * sign) > 0) {
                ...
                if(cur_facet == outside_neighbor_) {
                    ...
                    MadVoro::Exception::MadVoroException eo("Bad Walk");
                    throw eo;
                }
                break;
            }
...
template <typename PointT>
void Delaunay3D<PointT>::flip14(std::size_t point, std::size_t tetra)
{
    Tetrahedron toadd;
...
template <typename PointT>
bool Delaunay3D<PointT>::CheckCorrect(void)
{
    ...
    for(std::size_t i = 0; i < Ntetra; ++i) {
        ...
        for(std::size_t j = 0; j < 4; ++j) {
            ...
            if(T.neighbors[j] != outside_neighbor_) {
                ...
                if(loctemp >= 4)
                    throw MadVoro::Exception::MadVoroException("bad loctemp CheckCorrect");
                std::size_t other = 4;
                GetOppositePoint(tetras_[T.neighbors[j]], i, other);
                if(other >= 4)
                    throw MadVoro::Exception::MadVoroException("bad other CheckCorrect");
                b5_temp_[4] = points_[tetras_[T.neighbors[j]].points[other]];
                if((insphere(b5_temp_) < 0))
                    throw MadVoro::Exception::MadVoroException("bad insphere CheckCorrect");
            }
        }
    }
    return true;
...
template <typename PointT>
void Delaunay3D<PointT>::Clean(void)
{
    tetras_.clear();
    points_.clear();
    empty_tetras_.clear();
...
template <typename PointT>
void Delaunay3D<PointT>::ReleaseMemory(void)
{
    Clean();
    ContainerOps::release_container_memory(tetras_);
    ContainerOps::release_container_memory(points_);
    ContainerOps::release_container_memory(empty_tetras_);
    ContainerOps::release_container_memory(changed_tetras_);
    ContainerOps::release_container_memory(newTetras_);
}
